...
public class StageManagerScript : MonoBehaviour
{
    ...
    private AudioInfoDictionary<StageClip> Audio => AudioScript.I.StageAudio;
    ...
    private void Start()
    {
        ...
        IsBreak = () => !Audio.Dictionary[StageClip.Clear].Source.isPlaying;//終了条件を代入

        AllStageClearTime = new Interval();                             //Intervalをインスタンス化

        StageStartWaitInterval = new(time: StartWaitTime);             //Intervalをインスタンス化

        Player.Init(BulletList.GetObject, PlayerDeath);                      //プレイヤーの関数をセット
    ...
    private IEnumerator PlayerDeath(bool isNoLife)
    {
        ...
        var audio = AudioScript.I.StageAudio.Dictionary[StageClip.PlayerDeath];  //プレイヤー死亡Audio
    ...
    {
        Audio.Dictionary[StageClip.BGM].Source.Stop();         //BGMを停止

        c += () => Audio.Dictionary[StageClip.EyeCatch].Play();//アイキャッチSEを再生する関数を加算代入
    ...
    private void StageStart()
    {
        ...
        Audio.Dictionary[StageClip.BGM].Play();//BGMを再生
    ...
    private void Generate()
    {
        ...
        CurrentStage.Generate(EnemyList.GetObject, BulletList.GetObject, Player, OnAllEnemysDeath);//現在のステージを生成
    ...
    private void StageReSet()
    {
        ...
        CurrentStage.Clear(EnemyList);             //現在のステージをクリア

        foreach (var b in BulletList) b.Inactive();//弾のプールリスト分繰り返す/弾を非アクティブ
    }
}
